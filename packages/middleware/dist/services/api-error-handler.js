...
class ApiErrorHandler {
    constructor(retryConfig = {}, timeoutConfig = {}) {
    ...
    async withRetry(operation, config) {
        ...
        for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
            try {
                ...
                // Log retry attempt
                console.log(`[ApiErrorHandler] Retry attempt ${attempt + 1}/${retryConfig.maxRetries} after ${delay}ms`);
     ...
     */
    async withTimeoutAndRetry(operation, options) {
        return this.withRetry(() => this.withTimeout(operation, options?.timeoutMs), options?.retryConfig);
    }
    /**
