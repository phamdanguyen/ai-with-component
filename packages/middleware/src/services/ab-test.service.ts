...
export interface ABTestConfig {
  ...
  name: string;
  description: string;
  queryType?: string; // Optional: apply test only to specific query type
  componentType?: string; // Optional: apply test only to specific component type
...
export class ABTestService {
  ...
  private selectedVariants = new Map<string, string>(); // Cache variant selections
  ...
  recordResult(testId: string, variant: ABTestVariant, metrics: Record<string, unknown>): void {
    ...
    const result: ABTestResult = {
      ...
      success: (metrics.success as boolean) ?? true,
      executionTime: (metrics.executionTime as number) ?? 0,
      confidence: (metrics.confidence as number) ?? undefined,
      quality: (metrics.quality as number) ?? undefined,
      userSatisfaction: (metrics.userSatisfaction as number) ?? undefined,
      errorRate: (metrics.errorRate as number) ?? undefined,
      timestamp: new Date(),
    };
    ...
    console.log(
      `[ABTest] Recorded result for test '${testId}' variant '${variant.id}' - success: ${result.success}`
    );
  }
   ...
   */
  private calculateSignificance(variantStats: Array<{ successRate: number; results: number }>): boolean {
    ...
    if (maxSampleSize < minSampleSize) return false;
   ...
   */
  private calculatePValue(variantStats: Array<{ successRate: number; results: number }>): number {
    if (variantStats.length < 2) return 1.0;
   ...
   */
  private selectByWeight(variants: ABTestVariant[]): ABTestVariant {
   ...
   */
  private isTestValid(config: ABTestConfig): boolean {
