...
export class ErrorRecoveryService {
  ...
  static analyzeValidationError(
    ...
    const schemaErrors = errors.filter((e) =>
      e.toLowerCase().includes('schema validation') || e.toLowerCase().includes('validation error')
    );
    ...
    analysis.recoveryStrategies = this.getSuggestedStrategies(
      analysis.errorType,
      componentType,
      errors
    );
  ...
  private static getSuggestedStrategies(
    ...
    if (errorType === 'semantic_validation') {
      ...
      if (
        errors.some((e) =>
          e.includes('not found in') || e.includes('missing')
        )
      ) {
        strategies.push({
   ...
   */
  private static suggestFix(componentType: string, errors: string[]): string {
    // Chart errors
   ...
   */
  static getRecoveryPromptAmendment(
    errors: string[],
    componentType: string
  ): string {
    const amendment = `
...
5. Use simple, descriptive field names`;
  ...
  static isRecoverable(error: string, attemptNumber: number): boolean {
    ...
    const isSemanticError = recoverablePatterns.some((pattern) =>
  ...
  static logError(
    ...
    console.log(
      `${severity} Error Recovery [${componentType}] Attempt ${attemptNumber + 1}:`
    );
