...
export class MetricsService {
  ...
  generateReport(startTime?: number, endTime?: number): GenerationReport {
    ...
    const byComponent: Record<string, GenerationMetrics[]> = {};
    filtered.forEach((m) => {
      const type = m.componentType || 'unknown';
      if (!byComponent[type]) byComponent[type] = [];
      byComponent[type].push(m);
    });

    // Group by query type
    const byQuery: Record<string, GenerationMetrics[]> = {};
    filtered.forEach((m) => {
      const type = m.queryType || 'unknown';
      if (!byQuery[type]) byQuery[type] = [];
      byQuery[type].push(m);
    });
    ...
    return {
      ...
      byComponentType: Object.fromEntries(
        Object.entries(byComponent).map(([type, metrics]) => [
          type,
          {
            count: metrics.length,
            successRate:
              metrics.filter((m) => m.success).length / metrics.length, // decimal 0-1
            avgTime:
      ...
      byQueryType: Object.fromEntries(
        Object.entries(byQuery).map(([type, metrics]) => [
          type,
          {
            count: metrics.length,
            successRate:
  ...
  printReport(report?: GenerationReport): void {
    ...
    console.log('\n📈 Summary:');
    console.log(
      `  Total Requests:      ${rep.summary.totalRequests}`
    );
    console.log(
      `  Successful:          ${rep.summary.successfulRequests} (${rep.summary.successRate.toFixed(1)}%)`
    );
    console.log(
      `  Failed:              ${rep.summary.failedRequests}`
    );
    console.log(
      `  Avg Execution Time:  ${rep.summary.avgExecutionTime.toFixed(0)}ms`
    );
    ...
    if (Object.keys(rep.byComponentType).length > 0) {
      ...
      Object.entries(rep.byComponentType).forEach(([type, stats]) => {
        console.log(
          `  ${type}: ${stats.count} requests, ${stats.successRate.toFixed(1)}% success, ${stats.avgTime.toFixed(0)}ms avg`
        );
  ...
  private logMetric(metric: GenerationMetrics): void {
    ...
    console.log(
      `${status} [${metric.queryType}] ${component} ${time}${retries}`
    );
  ...
  private emptyReport(startTime: number, endTime: number): GenerationReport {
    return {
      // Flat properties
      totalMetrics: 0,
      successRate: 0,
