 ...
 */
export function validateChart(props: ChartProps): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateTable(props: TableProps): ValidationResult {
  const errors: string[] = [];
  ...
  if (missingKeys.size > 0) {
    errors.push(
      `Column keys not found in data: ${Array.from(missingKeys).join(', ')}`
    );
  }
 ...
 */
export function validateCard(props: CardProps): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateForm(props: FormProps): ValidationResult {
  const errors: string[] = [];
  ...
  for (let i = 0; i < props.fields.length; i++) {
    ...
    if (field.validation) {
      if (field.validation.minLength && field.validation.maxLength) {
        if (field.validation.minLength > field.validation.maxLength) {
          errors.push(
            `Field '${field.name}': minLength (${field.validation.minLength}) cannot be greater than maxLength (${field.validation.maxLength})`
          );
        }
      }
    }
 ...
 * - Each item has title
 */
export function validateList(props: ListProps): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateSlides(props: SlidesProps): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateReport(props: ReportProps): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateComponentFull(
  type: string,
  props: unknown
): ValidationResult {
  const errors: string[] = [];
  ...
  try {
    switch (type) {
      case 'chart':
        return validateChart(props as ChartProps);
      case 'table':
        return validateTable(props as TableProps);
      case 'card':
        return validateCard(props as CardProps);
      case 'form':
        return validateForm(props as FormProps);
      case 'list':
        return validateList(props as ListProps);
      case 'slides':
        return validateSlides(props as SlidesProps);
      case 'report':
        return validateReport(props as ReportProps);
      default:
        return {
          valid: false,
